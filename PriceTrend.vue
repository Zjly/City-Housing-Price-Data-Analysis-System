<template>
  <div class="tabs-group">
    <form @submit.prevent="getProvince">
      <Select v-model="cityChoose.selectProvince" style="width:100px" @on-change="changeProvince($event)" placeholder="请选择省份">
  ...
  <div class="listcontent">
    ...
    <ul class="toplist w600 ablue">
      <li class="ws_text huise"><span class="txuh" v-text="cityChoose.selectCity">新房房价</span><b class="diqu" v-text="cityChoose.selectCity">二手房房价</b></li>
    ...
    data() {
      return {
        ...
        trendMap: {
          ...
          yNum:['新房','二手房'],
          series:[
            {name:'新房',type:'line',stack: 'new',data:[]},
            {name:'二手房',type:'line',stack: 'esf',data:[]},
          ],   // 折线图数据
        }
      }
    },
    methods: {
      ...
      changeMap(event){
        ...
        self.$axios.get(path1, {
          getThree:{"city":self.cityChoose.selectCity}
        })
          .then(function (response) {
            console.log(response);
            self.trendMap.series = [{name:'新房',type:'line',stack: 'new',data:response.data.avg_new},{name:'新房',type:'line',stack: 'new',data:response.data.avg_esf}]
          }).catch(function (error) {
          console.log(error);
        });
        ...
        self.$axios.get(path2, {
          getOne:{"city":self.cityChoose.selectCity}
        })
          .then(function (response) {
            console.log(response);
            self.trendMap.series = [{name:'新房',type:'line',stack: 'new',data:response.data.avg_new},{name:'新房',type:'line',stack: 'new',data:response.data.avg_esf}]
